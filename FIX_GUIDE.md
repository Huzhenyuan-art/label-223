...
### 补充说明（2026-06-16 四次修复）：preloadPage 不足 + 子页面排位靠后

#### 现象
`safeNavigateTo` 已接入后，`admin`、`sensitiveWords` 等页面仍报 `Page has not been registered yet`。

#### 根因
1. `wx.preloadPage` 的 `success` 回调触发时，页面 JS 可能尚未执行完 `Page()` 注册
2. `admin`（原第 24 位）、`sensitiveWords`（原第 18 位）在 pages 数组末尾，按需注入优先级最低
3. 单次 `preloadPage` + 立即 `navigateTo` 无重试，超时即白屏

#### 修复
1. **增强 safeNavigateTo / safeRedirectTo / safeReLaunch**：
   - 预加载成功后延迟 150ms 再跳转
   - 捕获 `not been registered` / `timeout` 错误，最多重试 5 次（间隔 120ms）
2. **新增 `preloadSubPages()`**：`App.onLaunch` 与登录成功后，批量预加载全部 17 个子页面
3. **重排 app.json pages 顺序**：将 `member`、`sensitiveWords`、`auditLogs`、`admin` 等常用页前移（紧随 TabBar 页之后）

#### 完整子页面清单（均需预加载）
`detail`、`member`、`favorites`、`groups`、`chat`、`edit`、`sensitiveWords`、`auditLogs`、`admin`、`publicProfile`、`groupCreate`、`groupDetail`、`groupMembers`、`myItems`、`transactions`、`tagChannels`、`tagChannel`

### 补充说明（2026-06-16 五次修复）：ignoreDevUnusedFiles 过滤页面 JS（最终根因）

#### 现象
`preloadPage`、重试、重排 pages 均无效，`admin`、`sensitiveWords` 仍报 `Page has not been registered yet`。

#### 根因
`project.private.config.json` 中 **`ignoreDevUnusedFiles: true`**（过滤无依赖文件）。

微信开发者工具静态分析认为 `pages/admin/admin.js` 等页面 **仅通过 URL 字符串**（如 `safeNavigateTo('/pages/admin/admin')`）跳转，没有 `require` 引用链，于是将其标记为「无依赖文件」：
- 模拟器运行时 **不打包该 JS**
- 跳转时框架等待 `Page()` 注册 → 超时 → `has not been registered yet`

这与按需注入无关，是 **开发者工具构建配置** 导致页面 JS 被剔除。

#### 修复

1. **关闭无依赖文件过滤**（`project.private.config.json` + `project.config.json`）：
```json
"ignoreDevUnusedFiles": false,
"ignoreUploadUnusedFiles": false
```

2. **关闭无依赖文件过滤**（`project.private.config.json` + `project.config.json`）：
```json
"ignoreDevUnusedFiles": false,
"ignoreUploadUnusedFiles": false
```

> ⚠️ **勿在 app.js 中 require 页面 JS**（如 `bootstrap-pages.js`）。微信 3.16+ 禁止在 app 上下文调用 `Page()`，会导致全部页面注册失败。

3. **重命名 admin 页 `switchTab` → `handleAdminTab`**：避免与框架 `wx.switchTab` 命名冲突。

4. **修复 chat.js 编译错误**：`loadMessages` 内重复声明 `const userId` 会导致编译失败，后续页面均无法注册。

#### 开发者工具操作（必做）
1. 菜单 → **设置 → 项目设置 → 本地设置**
2. 取消勾选 **「上传时过滤无依赖文件」** 和 **「过滤无依赖文件」**
3. **清缓存 → 全部清除**，关闭并重新打开项目
4. 重新编译

#### 涉及文件
| 文件 | 改动 |
|------|------|
| `miniprogram/project.private.config.json` | ignoreDevUnusedFiles: false |
| `miniprogram/project.config.json` | 同上 |
| `miniprogram/pages/admin/admin.js` | switchTab → handleAdminTab |
| `miniprogram/pages/admin/admin.wxml` | 同步更新事件名 |
| `miniprogram/pages/chat/chat.js` | 删除 loadMessages 内重复 const userId |
| `miniprogram/app.js` | 移除 bootstrap-pages 引用（该方案与 3.16+ 不兼容） |
...
### 现象描述
管理员用户登录后，在岛屿空间（个人中心）页面找不到运营管理后台的入口，无法使用用户管理、帖子下架、私信检索、订单处理、营地咨询处理等运营功能。

### 根因分析
后端已实现完整的运营管理后台 API（用户检索/封禁、帖子下架/恢复、私信会话检索、订单确认、营地咨询处理、操作日志），但前端缺少两个关键环节：
1. **缺少管理后台页面**：小程序页面体系中没有运营管理后台页面
2. **缺少入口按钮**：个人中心（profile 页面）没有运营管理入口，管理员无法导航到管理功能

### 修复方案

#### 一、创建管理后台页面（admin）
新建 `pages/admin/` 目录，包含四个文件：
- `admin.js` - 页面逻辑，7 个 Tab（仪表盘、用户、帖子、私信、订单、咨询、日志）
- `admin.wxml` - 页面结构
- `admin.wxss` - 页面样式
- `admin.json` - 页面配置

**功能特性**：
- **仪表盘**：数据概览（总用户/今日新增/总帖子/总订单/待处理咨询/封禁用户/已下架帖子/总收入）、近7日注册趋势图
- **用户管理**：搜索/状态筛选、用户列表、封禁/解封操作（含原因输入）
- **帖子管理**：搜索/状态筛选、帖子列表、下架/恢复操作（含原因输入）
- **私信管理**：会话搜索、会话列表（含消息数和最后消息时间）
- **订单管理**：状态筛选、订单列表（含统计卡片）、手动确认订单
- **营地咨询**：状态筛选、咨询列表（含统计卡片）、标记已联系
- **操作日志**：模块筛选、日志列表（操作人、动作、原因）

#### 二、在个人中心添加管理员专属入口
在 profile 页面添加「运营管理」卡片，仅当 `isAdmin` 为 true 时显示，包含「管理后台」按钮。

#### 三、后端 API 返回 isAdmin 字段
在 `userController.js` 的 `getIsland` 方法中，profile 对象新增 `isAdmin` 字段，前端据此判断是否显示管理入口。

### 涉及文件清单

| 类型 | 文件路径 | 改动说明 |
|------|---------|---------|
| 新增 | `miniprogram/pages/admin/admin.js` | 管理后台页面逻辑 |
| 新增 | `miniprogram/pages/admin/admin.wxml` | 管理后台页面结构 |
| 新增 | `miniprogram/pages/admin/admin.wxss` | 管理后台页面样式 |
| 新增 | `miniprogram/pages/admin/admin.json` | 管理后台页面配置 |
| 修改 | `miniprogram/app.json` | 注册 admin 页面到 pages 数组 |
| 修改 | `miniprogram/config/index.js` | 添加管理后台 API 配置（9 个接口） |
| 修改 | `miniprogram/pages/profile/profile.wxml` | 添加「运营管理」卡片（仅管理员可见） |
| 修改 | `miniprogram/pages/profile/profile.js` | 新增 isAdmin 数据字段和 goAdminPanel 跳转方法 |
| 修改 | `backend/src/controllers/userController.js` | getIsland 的 profile 对象新增 isAdmin 字段 |

### 验证方法

#### 场景 A：管理员看到入口
1. 在数据库中将某用户的 `isAdmin` 字段设为 `true`
2. 使用该账号登录小程序
3. 进入「岛屿」Tab
4. 预期：在「我的频率」和「内容审核管理」之间看到「运营管理」卡片
5. 预期：卡片中包含「管理后台」按钮（蓝色主按钮样式）
...
#### 场景 C：管理后台功能验证
1. 点击「管理后台」按钮，进入管理页面
2. 预期：页面顶部有 7 个 Tab 切换器
3. 仪表盘 Tab：显示用户数、帖子数、订单数等统计
4. 用户 Tab：可搜索用户，封禁/解封操作
5. 帖子 Tab：可搜索帖子，下架/恢复操作
6. 私信 Tab：可搜索会话
7. 订单 Tab：可筛选订单状态，确认待支付订单
8. 咨询 Tab：可筛选咨询状态，标记已联系
9. 日志 Tab：可按模块筛选操作日志
...
## 问题：草稿箱页面点击草稿无任何反应，不跳转修改

### 现象描述
在草稿箱列表页，点击任一草稿卡片，页面没有任何反应，不跳转到编辑页，也没有任何错误提示。

### 根因分析
这是**微信小程序 TabBar 页面跳转限制**导致的问题，与之前「编辑按钮无反应」的根因本质相同。

完整链路诊断：

| 环节 | 问题 | 说明 |
|------|------|------|
|  跳转方法错误 | drafts.js 中使用了 safeNavigateTo | safeNavigateTo 底层调用 wx.navigateTo，该 API **不能跳转到 TabBar 页面**，会静默失败（不报错、不跳转、无任何提示） |
|  publish 是 TabBar 页面 | app.json tabBar 配置 | pages/publish/publish 被配置为底部 TabBar 的"发射"Tab，属于 TabBar 页面 |
|  URL 传参无效 | 使用 ?draftId=xxx 传参 | 即使跳转成功，wx.switchTab 也**不支持 URL 参数传递**，draftId 参数会丢失，publish 页拿不到要加载的草稿 ID |
|  onLoad 不被再次触发 | TabBar 页面切换时不重新 onLoad | 草稿 ID 如果放在 onLoad 中消费，第二次进入 TabBar 时 onLoad 不执行，永远加载不到草稿 |

### 修复方案
采用**参数暂存 Storage + switchTab 跳转 + onShow 消费参数**的方案。

#### 1. draft.js  新增参数传递基础设施
新增 3 个导出函数：
- setPublishPendingParams(params)  将待传参数写入 Storage（带 5 分钟 TTL）
- consumePublishPendingParams()  一次性消费参数（读取后删除，超时丢弃）
- switchTabToPublish(params)  封装完整跳转函数：先存参数，再 wx.switchTab

#### 2. drafts.js  改用 switchTabToPublish
goEditDraft / editDraft / publishDraft 都改为 switchTabToPublish({ draftId })；goCreateNew 改为 switchTabToPublish({ fresh: true })（fresh 表示重置页面）。

#### 3. publish.js  改造参数消费逻辑（核心修复）
- 引入 consumePublishPendingParams
- onLoad 不再立即 loadDraft()，改为设置 	his.pendingLoadDraftId
- onShow 开头优先 consumePublishPendingParams()，消费 draftId 或 fresh 参数
- 统一在 onShow 中判断 pendingLoadDraftId 并执行加载
- 新增 esetPageForFresh() 处理 fresh=true 场景

### 涉及文件清单
| 文件 | 改动说明 |
|------|---------|
| miniprogram/utils/draft.js | 新增 3 个 TabBar 传参辅助函数 |
| miniprogram/pages/drafts/drafts.js | 全部跳转为 switchTabToPublish |
| miniprogram/pages/publish/publish.js | onShow 优先消费 Storage 参数，延迟草稿加载 |

### 验证方法
1. 保存一篇草稿  岛屿  草稿箱  点击草稿  应正确跳转并加载草稿内容
2. 草稿箱空状态点「去发射频率」 应重置为空白发布页
3. 多次切换 Tab 后，草稿内容不会被错误重置
